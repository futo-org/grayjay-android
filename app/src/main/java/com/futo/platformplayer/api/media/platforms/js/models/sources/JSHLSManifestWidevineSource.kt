...
class JSHLSManifestWidevineSource : IHLSManifestWidevineSource, JSSource {
    ...
    constructor(plugin: JSClient, obj: V8ValueObject) : super(TYPE_HLS, plugin, obj) {
        val contextName = "HLSWidevineSource";
    ...
    override fun getLicenseRequestExecutor(): JSRequestExecutor? {
        return _obj.requireSourcePlugin("JSHLSManifestWidevineSource.getLicenseRequestExecutor").busy {
            if (!hasLicenseRequestExecutor || _obj.isClosed)
                return@busy null

            val result = V8Plugin.catchScriptErrors<Any>(_config, "[${_config.name}] JSHLSManifestWidevineSource", "obj.getLicenseRequestExecutor()") {
